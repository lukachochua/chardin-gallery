<x-layouts.admin>
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <h2 class="text-2xl font-bold mb-4">Edit Exhibition</h2>
            ...
            <form action="{{ route('admin.exhibitions.update', $exhibition) }}" method="POST"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $exhibition->title) }}"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('description', $exhibition->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="datetime-local" name="start_date" id="start_date"
                            value="{{ old('start_date', $exhibition->start_date->format('Y-m-d\TH:i')) }}"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        @error('start_date')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="end_date" id="end_date"
                            value="{{ old('end_date', $exhibition->end_date->format('Y-m-d\TH:i')) }}"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        @error('end_date')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                ...
                <div class="mb-4">
                    ...
                    @if ($exhibition->image)
                        <div class="mb-2">
                            <img src="{{ Storage::url($exhibition->image) }}" alt="Current exhibition image"
                        ...
                        @foreach ($artists as $artist)
                            <div class="flex items-center">
                                <input type="checkbox" name="artists[]" value="{{ $artist->id }}"
                                    ...
                                    {{ in_array($artist->id, old('artists', $exhibition->artists->pluck('id')->toArray())) ? 'checked' : '' }}>
                        ...
                        @foreach ($artworks as $artwork)
                            <div class="flex items-center">
                                <input type="checkbox" name="artworks[]" value="{{ $artwork->id }}"
                                    ...
                                    {{ in_array($artwork->id, old('artworks', $exhibition->artworks->pluck('id')->toArray())) ? 'checked' : '' }}>
